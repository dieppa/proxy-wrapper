...
then
    ...
    mvn clean verify sonar:sonar -Dsonar.projectKey=com.github.cloudyrock.proxy -Dsonar.organization=cloudyrock -Dsonar.host.url=https://sonarcloud.io -Dsonar.login=$SONAR_TOKEN
...
then
    ...
    mvn clean verify sonar:sonar -Dsonar.projectKey=com.github.cloudyrock.proxy -Dsonar.organization=cloudyrock -Dsonar.host.url=https://sonarcloud.io -Dsonar.login=$SONAR_TOKEN -Dsonar.pullrequest.base=master -Dsonar.pullrequest.key=$TRAVIS_PULL_REQUEST -Dsonar.pullrequest.branch=$TRAVIS_PULL_REQUEST_BRANCH
else
